...
class Predictor:
    ...
    def predict_test_set(self, batch_gen, return_results=True):
        ...
        for rank_ix, weight_path in enumerate(weight_paths):
            ...
            if self.net.mc_var:
                self.net.train()
                print('current mode:',self.net.training)
            ...
            with torch.no_grad():
                for _ in range(batch_gen['n_test']):
                    print('predicting : %i/%i'%(_,batch_gen['n_test']))
        ...
        if return_results:
            ...
            out_string = 'wcs_pred_boxes_list'
            with open(os.path.join(self.cf.fold_dir, '{}.pickle'.format(out_string)), 'wb') as handle:
                pickle.dump(list_of_results_per_patient, handle)
            end_time = time()
            # self.logger.info('end_time: {}'.format(end_time))
            self.logger.info('Running the inference took %f hours' % ((end_time - start_time)/3600))
...
def apply_wbc_to_patient(inputs):
    ...
    for bix, b in enumerate(in_patient_results_list):
        ...
        for cl in list(class_dict.keys()):
            ...
            if 'mc_var' in boxes[0][1].keys():
                box_mc_vars = np.array([b[1]['mc_var'] for b in boxes])
                box_mc_vars = box_mc_vars[:, None]
            else:
                box_mc_vars = None
            if 'pred_var' in boxes[0][1].keys():
                box_pred_vars = np.array([b[1]['pred_var'] for b in boxes])
                box_pred_vars = box_pred_vars[:, None]
            else:
                box_pred_vars = None

            if 0 not in box_scores.shape:
                keep_scores, keep_coords, keep_mc_vars, keep_pred_vars = weighted_box_clustering(
                    np.concatenate((box_coords, box_scores[:, None], box_center_factor[:, None],
                                    box_n_overlaps[:, None]), axis=1), box_patch_id, wcs_iou, n_ens, box_mc_vars, box_pred_vars)

                for boxix in range(len(keep_scores)):
                    to_append = {'box_type': 'det', 'box_coords': keep_coords[boxix],
                                 'box_score': keep_scores[boxix], 'box_pred_class_id': cl}
                    if len(keep_mc_vars) > 0:
                        to_append['mc_var'] = keep_mc_vars[boxix]
                    if len(keep_pred_vars) > 0:
                        to_append['pred_var'] = keep_pred_vars[boxix]
                    out_patient_results_list[bix].append(to_append)
...
def merge_2D_to_3D_preds_per_patient(inputs):
    ...
    for cl in list(class_dict.keys()):
        ...
        for kix, kz in zip(keep_ix, keep_z):
            to_append = {'box_type': 'det', 'box_coords': list(box_coords[kix]) + kz,
                         'box_score': box_scores[kix], 'box_pred_class_id': cl}
            if box_mc_vars is not None:
                to_append['mc_var'] = box_mc_vars[kix]
            if box_pred_vars is not None:
                to_append['pred_var'] = box_pred_vars[kix]
            out_patient_results_list.append(to_append)
...
def weighted_box_clustering(dets, box_patch_id, thresh, n_ens, box_mc_vars=None, box_pred_vars=None):
    """
    ...
    while order.size > 0:
        ...
        if box_mc_vars is not None:
            match_mc_vars = box_mc_vars[order[matches]]
            match_mc_vars *= match_score_weights
            avg_mc_var = np.sum(match_mc_vars) / denom
            if avg_score > 0.01:
                keep_mc_vars.append(avg_mc_var)


        if box_pred_vars is not None:
            match_pred_vars = box_pred_vars[order[matches]]
            match_pred_vars *= match_score_weights
            avg_pred_var = np.sum(match_pred_vars) / denom
            if avg_score > 0.01:
                keep_pred_vars.append(keep_pred_vars)
    ...
    return keep_scores, keep_coords, keep_mc_vars, keep_pred_vars



def nms_2to3D(dets, thresh, box_mc_vars=None, box_pred_vars=None):
    """
